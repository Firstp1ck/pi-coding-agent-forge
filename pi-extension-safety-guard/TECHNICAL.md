...
Bash prompts show the complete **Command**, with matched text marked as `>>> ... <<<`. Long or multiline commands start with an **Approval trigger** excerpt so the first match is not buried below the script. The excerpt includes the risk label, source line/column, and highlighted text. A count indicates additional triggers to inspect below. When no exact source location is available, the opening notice says so instead of inventing a highlight. Multiline commands include line numbers. Overlapping matches share one highlight; for example, the force and recursive removal rules appear as one `recursive force rm` risk. Distinct occurrences remain highlighted separately. Terminal highlights also use bold warning colors. A risky SQL heredoc marks `DROP TABLE`, not the heredoc opener. Parser failures, redirections, and interpreter names are not risk triggers by themselves.
...
Inline highlights map known risk matches back to the original command, including literal quoted arguments. Additional context-line risk excerpts are omitted when the complete command already has source highlights. The opening Approval trigger excerpt for long or multiline commands remains visible before the command. They remain available for overlong commands or when source highlights are unavailable. These excerpts use `!!!` for matched lines and `>>> pattern <<<` for matched text; their context-line settings still apply. A complete command preview is never shortened merely because you selected fewer excerpt context lines.
