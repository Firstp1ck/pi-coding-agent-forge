...
import { format, guidance, text, type Format, type Guidance, type Unit } from "./command.ts";
...
export interface BookInput { title: string; format?: string; style?: string; genre?: string; language?: string; brief?: string; guidance?: string }
export const UNIT_DIRS: Record<Unit, string> = { chapter: "chapters", scene: "scenes", volume: "volumes" };
export const PROJECT_DOCS = ["brief.md", "style.md", "outline.md", "continuity.md", "progress.md", "learning.md"];
const LIMIT = 4096;
...
export function validateBook(value: unknown, id: string): Book {
  ...
  return { version: 1, id, title: text(v.title, "Book title", 160), format: format(String(v.format)),
    ...(v.guidance !== undefined ? { guidance: guidance(v.guidance) } : {}), createdAt: v.createdAt };
}
...
export async function createProject(cwd: string, input: BookInput): Promise<Project> {
  ...
  const book: Book = { version: 1, id: slug(title), title, format: format(input.format ?? "novel"),
    ...(input.guidance !== undefined ? { guidance: guidance(input.guidance) } : {}), createdAt: new Date().toISOString() };
  const beginner = book.guidance === "beginner";
  ...
  const brief = text(input.brief ?? (beginner ? "Help me find a story idea and learn one small step at a time. I do not need a whole-book plan yet." : "Ask the author for a premise before proposing an outline."), "Brief", 4000, true);
  ...
  const docs: Record<string, string> = {
    ...
    "progress.md": beginner
      ? "# Writing checkpoint\n\n## Last saved work\n\nProject scaffold only. No manuscript has been drafted.\n\n## Current task\n\nFind a story idea and choose one small first exercise.\n\n## Next step\n\nRead learning.md. Build on any supplied idea, or offer a few simple starting points. Ask one question and wait for the author.\n\n## Open decisions\n\nThe author can write themselves, write together, or ask for a short example. No advanced choices are required yet.\n"
    ...
    ...(beginner ? { "learning.md": "# My writing notebook\n\n## How we work\n\nLearn one useful concept at a time using this story. Keep explanations short and ordinary. A working title and rough idea are enough.\n\n## My choices\n\nAsk whether I want to try writing, write together, or see a short example when it becomes relevant. Do not assume a preference yet.\n\n## Current exercise\n\nNot chosen yet. Start with the idea I supplied, or help me find one.\n\n## Tried so far\n\nNo exercises completed yet. Record actual attempts and feedback, not assumed mastery.\n\n## Next small step\n\nChoose one story seed. Save the next question or exercise after we work on it, so I can continue later.\n" } : {}),
  };
  try {
