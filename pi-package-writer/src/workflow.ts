...
import type { Request } from "./command.ts";
import type { Project } from "./store.ts";

const PACKAGE_ROOT = dirname(dirname(fileURLToPath(import.meta.url)));
export const SKILLS = [
  "writer-workflow", "writer-brainstorm", "writer-outline", "writer-characters",
...
export function skillsFor(request: Request, project: Project): string[] {
  ...
  const actionSkills: Record<string, string[]> = {
    new: request.unit === "book" || request.unit === "volume" ? ["writer-brainstorm", "writer-outline"] : ["writer-prose", "writer-continuity"],
...
export function buildPrompt(request: Request, project: Project, files: string[], target?: string, source?: string): string {
  const actions: Record<string, string> = {
    new: request.unit === "book"
      ? "Plan a new book. Clarify only missing decisions that affect the opening. Propose an outline and style calibration; stop for author approval before manuscript drafting."
      : request.unit === "volume"
        ? "Plan one new volume, preserving series canon and its own central conflict. Save a proposed volume outline in the reserved target; do not draft the volume."
        : `Create one ${request.unit}. Read the surrounding work first. If direction is approved, plan briefly and draft the requested unit. If essential choices remain unresolved, ask before inventing them.`,
  ...
  const data = {
    ...
    title: project.book.title, format: project.book.format, projectRoot: project.root,
